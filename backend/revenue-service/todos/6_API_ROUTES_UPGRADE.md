# API Routes Upgrade - Revenue Service

## 📊 การปรับปรุง API Routes
...
##### **POST /api/revenue/batches/:id/process**
```typescript
// ประมวลผล batch
POST /api/revenue/batches/batch-123/process
...
// Response
{
    ...
    "errors": [
      {
        ...
        "error": "ไฟล์ไม่ผ่านการตรวจสอบ",
        "details": ["Invalid file format", "Missing required fields"]
      }
    ],
    "progress": {
      "batchId": "batch-123",
  ...
  "message": "ประมวลผล batch สำเร็จ (4/5 ไฟล์)",
  "timestamp": "2024-01-15T10:45:00Z"
}
```
...
- **BatchService**: ใช้ BatchService แทนการเรียก databaseService โดยตรง
- **ValidationService**: เพิ่ม validation สำหรับ batch operations
- **Error Handling**: ใช้ BatchServiceError สำหรับ batch-specific errors

#### **4. Middleware Enhancement**
- **validateBatchId**: เพิ่ม validation สำหรับ batch ID
- **Rate Limiting**: ใช้ rate limiting สำหรับ batch operations
- **Request Validation**: เพิ่ม validation สำหรับ batch requests
...
// ประมวลผล batch
const processResponse = await fetch('/api/revenue/batches/batch-123/process', {
  method: 'POST'
});
```
...
- ✅ **Service Integration**: ปรับปรุงสำเร็จ
- ✅ **Middleware Enhancement**: เพิ่มสำเร็จ
