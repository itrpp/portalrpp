...
function normalizeNumericId(value: unknown): number | null {
  ...
  if (!Number.isInteger(asNumber) || asNumber <= 0) throw new Error('INVALID_ID_TYPE');

  return asNumber;
}

type HrdIdType = 'personType' | 'position' | 'department' | 'departmentSub' | 'departmentSubSub';

async function assertHrdIdExists(type: HrdIdType, id: number | null): Promise<void> {
  if (id === null) return;
  let count = 0;
 ...
 */
export async function parseAndValidateProfileUpdate(payload: unknown): Promise<ProfileUpdateData> {
  ...
  const personTypeId = 'personTypeId' in p ? normalizeNumericId(p.personTypeId) : undefined;
  const positionId = 'positionId' in p ? normalizeNumericId(p.positionId) : undefined;
  const departmentId = 'departmentId' in p ? normalizeNumericId(p.departmentId) : undefined;
  const departmentSubId =
    'departmentSubId' in p ? normalizeNumericId(p.departmentSubId) : undefined;
  const departmentSubSubId =
    'departmentSubSubId' in p ? normalizeNumericId(p.departmentSubSubId) : undefined;

  if ('personTypeId' in p && personTypeId === null) throw new Error('PERSON_TYPE_REQUIRED');
  if ('positionId' in p && positionId === null) throw new Error('POSITION_REQUIRED');
  if ('departmentId' in p && departmentId === null) throw new Error('DEPARTMENT_ID_REQUIRED');
  ...
  await Promise.all([
    'personTypeId' in p ? assertHrdIdExists('personType', personTypeId ?? null) : Promise.resolve(),
    'positionId' in p ? assertHrdIdExists('position', positionId ?? null) : Promise.resolve(),
    'departmentId' in p ? assertHrdIdExists('department', departmentId ?? null) : Promise.resolve(),
  ...
  if ('departmentSubId' in p) updateData.departmentSubId = departmentSubId ?? null;
