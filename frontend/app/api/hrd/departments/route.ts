...
import { listDepartments, createDepartment, CreateDepartmentError } from '@/lib/hrd';
...
export async function POST(request: Request) {
  try {
    ...
    const data = await createDepartment(typeof name === 'string' ? name : '', active);
    ...
    const message = error instanceof Error ? error.message : 'เกิดข้อผิดพลาดในการสร้างกลุ่มภารกิจ';
