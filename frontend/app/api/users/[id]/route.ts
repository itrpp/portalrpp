...
async function assertHrdIdExists(
  type: 'personType' | 'position' | 'department' | 'departmentSub' | 'departmentSubSub',
  id: number | null,
) {
  if (id === null) {
    return;
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    try {
      ...
      const personTypeId =
        'personTypeId' in payload ? normalizeNumericId(payload.personTypeId) : undefined;
      const positionId =
        'positionId' in payload ? normalizeNumericId(payload.positionId) : undefined;
      const departmentId =
        'departmentId' in payload ? normalizeNumericId(payload.departmentId) : undefined;
      const departmentSubId =
        'departmentSubId' in payload ? normalizeNumericId(payload.departmentSubId) : undefined;
