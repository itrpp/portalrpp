...
export async function PUT(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (requestData.roomCount !== undefined) {
      protoRequest.room_count = requestData.roomCount === null ? 0 : requestData.roomCount;
    }
    // ส่ง bedCount ถ้ามีค่า (รวมถึง null เพื่อลบข้อมูล - ส่ง 0 เพื่อให้ backend แปลงเป็น null)
    if (requestData.bedCount !== undefined) {
      protoRequest.bed_count = requestData.bedCount === null ? 0 : requestData.bedCount;
    }
    // ส่ง status ถ้ามีค่า
    ...
    // เรียก gRPC service
    const response = await callPorterService<any>('UpdateFloorDepartment', protoRequest);
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'UPDATE_FAILED',
          message: response.error_message || 'ไม่สามารถอัปเดตคลีนิก/หอผู้ป่วยได้',
        },
        { status: 400 },
      );
    }
