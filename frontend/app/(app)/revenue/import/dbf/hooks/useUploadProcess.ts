...
export const useUploadProcess = () => {
  ...
  const generateBatchId = useCallback(async (): Promise<string> => {
    const now = new Date();
    ...
    async (
      ...
      try {
        ...
        if (uploadResponse.success) {
          ...
          throw new Error(
            uploadResponse.data?.message || "เกิดข้อผิดพลาดในการอัปโหลด",
          );
        }
      } catch {
  ...
  const validateSingleFile = useCallback(
    async (
      ...
      try {
        ...
        const validateResult = await api.validateRevenueFileById(
          session,
          serverFile.id,
        );
        ...
        if (validateResult.success && validateResult.data) {
          ...
          const fileSizeMB = actualFileSize
            ? (actualFileSize / (1024 * 1024)).toFixed(2)
            : "ไม่ทราบ";
          ...
          const errorMsg =
            validateResult.data?.message || "เกิดข้อผิดพลาดในการตรวจสอบไฟล์";

          detailedMessage = errorMsg;

          if (
            validateResult.data?.errors &&
            validateResult.data.errors.length > 0
          ) {
  ...
  const matchFilesForValidation = useCallback(
    (serverFiles: any[], uploadedFiles: UploadedFile[]) => {
      ...
      for (const uploadedFile of uploadedFiles) {
        const matchingFile = serverFiles.find((f) => {
          const fileName =
            f.fileName ||
            (f as any).originalName ||
            (f as any).filename ||
            `file-${f.id}`;

          return (
            uploadedFile.file.name === fileName ||
  ...
  const uploadFiles = useCallback(
    async (
      ...
      try {
        ...
        // const batchId = "cmgyl7b8s0004irn7ohdo03k1";
        ...
        for (let i = 0; i < uploadedFiles.length; i += CONCURRENT_UPLOADS) {
          ...
          const batchResults = await Promise.allSettled(
            batch.map((file) =>
              uploadSingleFile(file, batchId, updateUploadedFile),
            ),
          );
        ...
        if (successCount > 0) {
          ...
          if (serverFiles.length > 0) {
            const filesToValidate = matchFilesForValidation(
              serverFiles,
              uploadedFiles,
            );
