...
export const useFileUpload = (maxFileSize: number, maxFiles: number) => {
  ...
  const generateFileChecksum = useCallback(
    async (file: File): Promise<string> => {
      return new Promise((resolve, reject) => {
        ...
        reader.onerror = () =>
          reject(new Error("Failed to read file for checksum"));
  ...
  const validateDBFFile = useCallback(
    (file: File, uploadedFilesCount: number): boolean => {
    ...
    async (
      ...
      for (const file of Array.from(files)) {
        ...
        try {
          ...
          setErrorMessage(
            `ไม่สามารถสร้าง checksum สำหรับไฟล์ ${file.name} ได้`,
          );
          hasError = true;
          break;
        }
