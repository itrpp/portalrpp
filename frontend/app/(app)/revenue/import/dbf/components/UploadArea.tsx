        ...
        <div className='space-y-4'>
            ...
            >
                ...
                <p className='text-sm text-default-600 mb-3'>
                    รองรับไฟล์ DBF ขนาดสูงสุด {formatFileSize(maxFileSize)} จำนวนสูงสุด {maxFiles} ไฟล์
                </p>
                <Button
                    color='primary'
                    variant='solid'
                ...
                <div className='space-y-2'>
                    ...
                    <div className='space-y-2 max-h-60 overflow-y-auto' aria-label="รายการไฟล์ที่เลือก">
                        {uploadedFiles.map((file, index) => (
                            ...
                            >
                                <div className='flex items-center space-x-3 flex-1' aria-label={`ข้อมูลไฟล์: ${file.file?.name || 'Unknown'}`}>
                                    <FileTextIcon className='h-5 w-5 text-primary-600 dark:text-primary-400' />
                                    <div className='flex-1 min-w-0'>
                                        <p className='font-medium text-foreground truncate'>{file.file?.name || 'Unknown'}</p>
                                        <p className='text-sm text-default-600'>{file.fileSize ? formatFileSize(file.fileSize) : 'Unknown'}</p>
                                        {file.checksum && (
                                ...
                                <div className='flex items-center space-x-2' aria-label="การดำเนินการกับไฟล์">
                                    ...
                                    {file.status === 'uploading' && (
                                        <div className="flex items-center space-x-2" aria-label={`กำลังอัปโหลด ${file.file?.name || 'Unknown'}: ${file.progress || 0}%`}>
                                            <UploadIcon className='h-4 w-4 text-primary-500 animate-pulse' />
                                            <div className='w-20 h-2 bg-default-200 rounded overflow-hidden'>
                                                <div className='h-2 bg-primary-500' style={{ width: `${file.progress || 0}%` }} />
                                            </div>
                                            <span className="text-xs text-default-600">{file.progress || 0}%</span>
                                        </div>
                                    )}
