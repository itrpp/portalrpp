...
import { BatchStatus, UploadBatch } from "@/types/revenue";
      ...
      <CardBody>
        {isLoading ? (
          ...
          <Table aria-label="ประวัติการอัปโหลด Batch">
            ...
            <TableBody
              emptyContent={"ยังไม่มีประวัติการอัปโหลด DBF Files"}
              items={uploadBatches}
            >
              {(batch) => (
                <TableRow key={batch.id}>
                  <TableCell className="whitespace-nowrap overflow-hidden text-ellipsis max-w-32">
                    <div
                      aria-label={`ชื่อ batch: ${batch.batchName}`}
