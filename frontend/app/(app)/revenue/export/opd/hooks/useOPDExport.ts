...
export const useOPDExport = () => {
      ...
      if (response.success && response.data) {
        const allBatches: UploadBatch[] = response.data.batches.map(
          (batch: any) => ({
            ...
            status: batch.status,
            processingStatus:
              batch.processingStatusOpd || batch.processingStatus || "pending",
            exportStatus:
              batch.exportStatusOpd || batch.exportStatus || "not_exported",
  ...
  const isProcessed = useCallback((batch: UploadBatch): boolean => {
    const processingStatus =
      (batch as any).processingStatusOpd || batch.processingStatus || "pending";
  ...
  const isExported = useCallback((batch: UploadBatch): boolean => {
    const exportStatus =
      (batch as any).exportStatusOpd || batch.exportStatus || "not_exported";
  ...
  const handleEdit = useCallback(
    async (batchId: string) => {
      ...
      try {
        addToast({
          title: "กำลังประมวลผลปรับปรุงข้อมูล OPD...",
          color: "primary",
        });
          ...
          const updated = uploadBatches.map((b) =>
            b.id === batchId
              ? {
                  ...
                  processingStatusOpd: "completed" as const,
                }
              : b,
          );
        ...
        addToast({
          title: "เกิดข้อผิดพลาดระหว่างการประมวลผล",
          color: "danger",
        });
      }
    },
