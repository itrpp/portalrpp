...
export default function RevenuePage() {
  ...
  const [newRevenue, setNewRevenue] = useState({
    ...
    payerType: '',
    description: '',
    collectionDate: new CalendarDate(new Date().getFullYear(), new Date().getMonth() + 1, new Date().getDate()),
    receiptNumber: '',
    notes: '',
  });
  ...
  const handleAddRevenue = () => {
    ...
    const newRevenueItem: RevenueCollection = {
      ...
      collectionDate: format(newRevenue.collectionDate.toDate('Asia/Bangkok'), 'yyyy-MM-dd'),
      status: 'PENDING',
  ...
  // แก้ไข: ใส่วงเล็บรอบ argument ของ arrow function ตามมาตรฐาน ESLint
  const filteredRevenues = revenues.filter((revenue) => {
    const matchesSearch =
      revenue.payerName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className='container mx-auto p-6 space-y-6'>
      ...
      <div className='flex justify-between items-center'>
        <div>
          ...
          <p className='text-gray-600 mt-2'>
            จัดการและติดตามรายการรายได้ทั้งหมด
          </p>
        </div>
        <Button
          color='primary'
      ...
      <div className='grid grid-cols-1 md:grid-cols-4 gap-6'>
        <Card className='hover:shadow-lg bg-background/90 backdrop-blur-lg border border-default-200 dark:border-default-700'>
        ...
        <Card className='hover:shadow-lg bg-background/90 backdrop-blur-lg border border-default-200 dark:border-default-700'>
          ...
          <CardBody>
            <div className='text-2xl font-bold'>
              {revenues.filter((r) => r.status === 'PENDING').length}
            </div>
        ...
        <Card className='hover:shadow-lg bg-background/90 backdrop-blur-lg border border-default-200 dark:border-default-700'>
          ...
          <CardBody>
            <div className='text-2xl font-bold'>
              {revenues
                .filter(
                  (r) => r.collectionDate === format(new Date(), 'yyyy-MM-dd')
                )
        ...
        <Card className='hover:shadow-lg bg-background/90 backdrop-blur-lg border border-default-200 dark:border-default-700'>
          ...
          <CardBody>
            <div className='text-2xl font-bold'>
              {new Set(revenues.map((r) => r.payerName)).size}
            </div>
                ...
                <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
                  ...
                  <div className='space-y-2'>
                    ...
                    >
                      ...
                      <SelectItem key='TAX' textValue='TAX'>
                        ภาษี
                      </SelectItem>
                      <SelectItem key='FEE' textValue='FEE'>
                        ค่าธรรมเนียม
                      </SelectItem>
                      <SelectItem key='FINE' textValue='FINE'>
                        ค่าปรับ
                      </SelectItem>
                      <SelectItem key='LICENSE' textValue='LICENSE'>
                        ใบอนุญาต
                      </SelectItem>
                  ...
                  <div className='space-y-2'>
                    ...
                    >
                      ...
                      <SelectItem key='COLLECTED' textValue='COLLECTED'>
                        จัดเก็บแล้ว
                      </SelectItem>
                      <SelectItem key='OVERDUE' textValue='OVERDUE'>
                        เกินกำหนด
                      </SelectItem>
                  ...
                  <TableBody>
                    {filteredRevenues.map((revenue) => (
                      <TableRow key={revenue.id}>
                        ...
                        <TableCell>
                          {format(
                            new Date(revenue.collectionDate),
                            'dd/MM/yyyy',
                            { locale: th }
                          )}
                        </TableCell>
          ...
          <ModalBody>
            <div className='grid grid-cols-2 gap-4'>
              ...
              <div className='space-y-2'>
                ...
                >
                  <SelectItem key='INDIVIDUAL' textValue='INDIVIDUAL'>
                    บุคคล
                  </SelectItem>
              ...
              <div className='space-y-2'>
                ...
                >
                  ...
                  <SelectItem key='DEBIT_CARD' textValue='DEBIT_CARD'>
                    บัตรเดบิต
                  </SelectItem>
